...
- 新拖入批次替换旧批次。
- 防止同一批次重复启动；启动门与选择状态独立，替换选择不会提前隐藏旧启动进度，并忽略旧异步任务对新选择的迟到结果。
...
- `WorkspaceOpenCoordinator`：接收批次、选择目标、状态转换、独立启动活动/目标投影、成功清空、失败保留、重复启动保护与选择版本保护。
...
1. 窗口拖放或启动参数产生原始路径列表。
2. `FolderBatchPlanner` 在任何存在性探测前拒绝 UNC/设备命名空间，使用 `Path.GetFullPath` 规范化本地/盘符路径，规范化非根路径的末尾目录分隔符，通过 `Directory.Exists` 验证目录，并以 `StringComparer.OrdinalIgnoreCase` 去重；结果保留原输入顺序。
3. 协调器递增选择版本，使用新批次替换旧批次，并显示有效与无效数量。
4. UI 显示三个目标按钮。可用性由平台启动器查询，未安装目标显示“未安装”。
5. 用户点击一个目标后，协调器复制当前批次和版本，独立持有启动门及活动目标，并进入 `Launching`。再次点击不会重复启动；期间的新批次可以把选择状态改为 `Ready`，但不会释放启动门或隐藏旧目标进度。
...
- 仅末尾斜杠/反斜杠不同的非根目录也视为重复，盘符根目录保持不变。
- 原始或规范化后的 UNC（`\\server\share`、`//server/share`）及设备命名空间（`\\?\`、`\\.\`）在任何存在性探测前以明确原因拒绝，避免非预期 SMB/NTLM 访问；已经建立的映射驱动器以普通盘符路径处理。
...
1. PATH 中可验证的 `code.exe`/`Code.exe`；若安装的 `bin` 目录仅暴露 `code.cmd`，只用它定位并验证相邻安装根目录的 `Code.exe`，绝不启动 `.cmd`。
...
所有来源进入同一组 Ultimate/Community/版本/启动器/来源排序；未知版本的 PATH 项不会优先于已知 Ultimate，但在它是唯一有效候选时仍可使用。PATH 启动器通过相邻 `product-info.json` 尽可能识别版本。
...
- 便携 ZIP 结构、精确校验 sidecar、实际 ZIP SHA-256 与解压后内容/PDB 约束验证。
...
5. 生成同名 `.sha256`，重新计算实际 ZIP 哈希，把 ZIP 解压到唯一临时目录并复用便携内容/PDB 检查；验证成功后才上传二者为 CI artifact。
